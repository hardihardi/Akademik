...
<?= $this->section('content') ?>
    ...
    <div class="mb-6 lg:mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 lg:gap-8 animate-in fade-in slide-in-from-top-4 duration-700">
        <div>
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="px-4 py-1.5 bg-indigo-50 text-indigo-700 text-[10px] font-black rounded-xl border border-indigo-100 uppercase tracking-widest mt-2 shrink-0">ID: #<?= $teacher['id'] ?></span>
            </div>
            <p class="text-slate-500 mt-2 font-medium text-xs lg:text-sm">Modifikasi informasi personal dan status kepegawaian guru.</p>
        </div>
        <div>
    ...
    <div class="pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <form action="<?= base_url('teachers/' . $teacher['id']) ?>" method="post" enctype="multipart/form-data" class="space-y-6 lg:space-y-8">
            <?= csrf_field() ?>
            ...
            <div class="bg-white rounded-2xl lg:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                <div class="p-6 lg:p-8 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Informasi Dasar & Foto Profil</p>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col lg:flex-row gap-10 lg:gap-14">
                            ...
                            <div class="relative group">
                                <div class="w-40 h-40 lg:w-48 lg:h-48 rounded-2xl lg:rounded-[2rem] bg-slate-100 border-4 border-white shadow-inner flex items-center justify-center overflow-hidden" id="photo-preview">
                                    <?php if(!empty($teacher['photo'])): ?>
                                        <img src="<?= get_photo_url($teacher['photo']) ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                    <?php else: ?>
                        ...
                        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                            ...
                            <div class="md:col-span-2 space-y-3">
                                ...
                                <input class="w-full px-5 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-bold text-slate-800 placeholder:text-slate-300 text-sm" id="full_name" name="full_name" type="text" value="<?= $teacher['full_name'] ?>" placeholder="Contoh: Dr. Herman Saputra, M.Pd" required>
                            </div>
                            <div class="md:col-span-2 space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest" for="phone">
                                    Nomor Telepon / WhatsApp
                                </label>
                                <input class="w-full px-5 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-emerald-50 focus:border-emerald-500 transition-all font-bold text-slate-800 placeholder:text-slate-300 text-sm" id="phone" name="phone" type="text" value="<?= $teacher['phone'] ?>" placeholder="08xxxxxxxxxx">
                            </div>
                            <div class="md:col-span-2 space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest" for="address">
                                    Alamat Lengkap Domisili
                                </label>
                                <textarea class="w-full px-5 py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-bold text-slate-800 placeholder:text-slate-300 text-sm min-h-[120px]" id="address" name="address" placeholder="Tuliskan alamat lengkap..."><?= $teacher['address'] ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 lg:p-8 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-end gap-4">
                    ...
                    <p class="text-[9px] lg:text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none shrink-0">TERAKHIR DIUPDATE: <span class="text-slate-400 font-mono"><?= date('H:i d/m/y', strtotime($teacher['updated_at'] ?? $teacher['created_at'])) ?></span></p>
                </div>
            </div>
        </form>
    </div>
