...
<?= $this->section('content') ?>
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-6 uppercase tracking-widest animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            <nav class="flex mb-3 lg:mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3 text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em]">
                    ...
                    <li class="flex items-center">
                        <svg class="w-3 h-3 text-slate-300 mx-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="text-indigo-600 uppercase font-black">Tambah Role</span>
                    </li>
                </ol>
            </nav>
            <h1 class="text-2xl md:text-3xl lg:text-4xl font-black text-slate-800 tracking-tight leading-tight uppercase"><?= $title ?></h1>
            <p class="text-slate-500 mt-2 font-medium text-xs lg:text-sm">Definisikan peran baru dan tentukan batasan hak akses sistem.</p>
        </div>
        <div>
    ...
    <div class="max-w-4xl pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <form action="<?= base_url('roles/store') ?>" method="post" class="space-y-6 lg:space-y-8">
            <?= csrf_field() ?>
                ...
                <div class="p-6 lg:p-10 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <input class="w-full px-5 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-bold text-slate-800 placeholder:text-slate-300 text-sm" id="name" name="name" type="text" placeholder="Contoh: staff_akademik" required>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <input class="w-full px-5 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-bold text-slate-800 placeholder:text-slate-300 text-sm" id="description" name="description" type="text" placeholder="Tujuan atau fungsi role ini">
                        </div>
                    </div>
                                ...
                                <?php foreach($permissions as $perm): ?>
                                    <label class="flex items-center p-4 bg-white rounded-xl border border-slate-100 hover:border-indigo-200 hover:shadow-sm transition-all cursor-pointer group">
                                        <div class="relative flex items-center justify-center">
                                            <input type="checkbox" name="permissions[]" value="<?= $perm['id'] ?>" id="perm_<?= $perm['id'] ?>" class="peer w-5 h-5 opacity-0 absolute cursor-pointer">
                                        ...
                                        <div class="ml-4">
                                            <p class="text-xs font-black text-slate-700 uppercase group-hover:text-indigo-600 transition-colors"><?= $perm['name'] ?></p>
                                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter"><?= $perm['description'] ?></p>
                                        </div>
                                    </label>
                                <?php endforeach; ?>
                            </div>
