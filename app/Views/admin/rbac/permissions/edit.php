...
<?= $this->section('content') ?>
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-6 uppercase tracking-widest animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            <nav class="flex mb-3 lg:mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3 text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em]">
                    ...
                    <li class="flex items-center">
                        <svg class="w-3 h-3 text-slate-300 mx-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="text-indigo-600 uppercase font-black">Edit Permission</span>
                    </li>
                </ol>
            </nav>
            <div class="flex items-center gap-4">
                ...
                <span class="px-4 py-1.5 bg-indigo-50 text-indigo-700 text-[10px] font-black rounded-xl border border-indigo-100 uppercase tracking-widest mt-2 shrink-0">ID: #<?= $permission['id'] ?></span>
            </div>
            <p class="text-slate-500 mt-2 font-medium text-xs lg:text-sm">Perbarui definisi hak akses sistem secara granular.</p>
        </div>
        <div>
    ...
    <div class="pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <form action="<?= base_url('permissions/update/' . $permission['id']) ?>" method="post" class="space-y-6 lg:space-y-8">
            <?= csrf_field() ?>
            ...
            <div class="bg-white rounded-2xl lg:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden text-slate-800">
                <div class="p-6 lg:p-10 space-y-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <input class="w-full px-5 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-bold text-slate-800 placeholder:text-slate-300 text-sm uppercase" id="name" name="name" type="text" value="<?= $permission['name'] ?>" required>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <input class="w-full px-5 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-bold text-slate-800 placeholder:text-slate-300 text-sm" id="description" name="description" type="text" value="<?= $permission['description'] ?>">
                    </div>
                </div>
                ...
                <div class="p-6 lg:p-8 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-end gap-4">
                    ...
                    <p class="text-[9px] lg:text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none shrink-0">TERAKHIR DIUPDATE: <span class="text-slate-400 font-mono"><?= date('H:i d/m/y', strtotime($permission['updated_at'] ?? $permission['created_at'])) ?></span></p>
                </div>
            </div>
        </form>
    </div>
