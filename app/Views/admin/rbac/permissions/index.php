    ...
    <div class="mb-6 lg:mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 lg:gap-8">
        <div>
            <div class="flex items-center gap-2 text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 lg:mb-4">
                ...
                <span class="text-indigo-600 uppercase">HAK AKSES / PERMISSIONS</span>
            </div>
            <h1 class="text-2xl md:text-3xl lg:text-4xl font-black text-slate-800 tracking-tight leading-tight uppercase"><?= $title ?></h1>
            <p class="text-slate-500 mt-2 font-medium text-xs lg:text-sm">Kelola spesifik perizinan akses fitur pada sistem.</p>
        </div>
        <div>
            <a href="<?= base_url('permissions/create') ?>" class="w-full sm:w-auto inline-flex items-center justify-center px-6 lg:px-8 py-3 lg:py-4 rounded-xl lg:rounded-2xl bg-indigo-600 text-white font-black text-[10px] lg:text-xs tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 hover:shadow-indigo-200 hover:-translate-y-0.5 transition-all duration-300 group">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50">
                        ...
                        <th class="p-5 lg:p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">Permission Name</th>
                        <th class="p-5 lg:p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">Deskripsi Kegunaan</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-all duration-300">
                        <td class="p-5 lg:p-6 text-xs font-bold text-slate-400"><?= $perm['id'] ?></td>
                        <td class="p-5 lg:p-6">
                            <span class="text-indigo-600 font-mono text-[11px] font-black bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 group-hover:bg-indigo-600 group-hover:text-white transition-all uppercase"><?= $perm['name'] ?></span>
                        </td>
                        <td class="p-5 lg:p-6">
                            <span class="text-sm font-medium text-slate-500"><?= $perm['description'] ?: 'Tidak ada deskripsi' ?></span>
                        </td>
                        <td class="p-5 lg:p-6 text-right">
                            <div class="flex items-center justify-end gap-2 text-sm font-medium tracking-widest">
                                <a href="<?= base_url('permissions/edit/' . $perm['id']) ?>" class="p-2.5 bg-slate-50 text-slate-400 hover:bg-white hover:text-emerald-600 hover:shadow-md rounded-xl transition-all duration-300" title="Edit">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </a>
                                <form action="<?= base_url('permissions/delete/' . $perm['id']) ?>" method="post" class="inline" onsubmit="confirmDelete(event, 'Apakah Anda yakin ingin menghapus permission ini?');">
                                    <?= csrf_field() ?>
    ...
    <div class="md:hidden space-y-3 mb-10 uppercase tracking-widest">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 group">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[10px] font-black text-slate-400">#<?= $perm['id'] ?></span>
                <div class="flex items-center gap-2">
                    <a href="<?= base_url('permissions/edit/' . $perm['id']) ?>" class="p-2 bg-slate-50 text-emerald-600 rounded-lg">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                    </a>
                    <form action="<?= base_url('permissions/delete/' . $perm['id']) ?>" method="post" class="inline" onsubmit="confirmDelete(event, 'Hapus permission ini?');">
                        <?= csrf_field() ?>
            ...
            <h5 class="text-[11px] font-mono font-black text-indigo-600 mb-2"><?= $perm['name'] ?></h5>
            <p class="text-[10px] text-slate-400 leading-relaxed font-medium normal-case"><?= $perm['description'] ?: 'Tidak ada deskripsi' ?></p>
        </div>
        <?php endforeach; ?>
    </div>
