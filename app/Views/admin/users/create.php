    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-6 uppercase tracking-widest animate-in fade-in slide-in-from-top-4 duration-500">
            <div>
                ...
                <p class="text-slate-500 mt-2 font-medium normal-case">Buat akun pengguna baru dalam sistem.</p>
            </div>
                ...
                <form action="<?= base_url('users/store') ?>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-col items-center mb-16">
                        <div class="relative group">
                            <div id="photo-preview" class="w-32 h-32 rounded-[2.5rem] bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-all duration-500 group-hover:border-indigo-400 group-hover:bg-indigo-50/30 shadow-inner">
                                <svg class="w-12 h-12 text-slate-300 group-hover:text-indigo-400 transition-colors duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                            </div>
                        ...
                        <div class="mt-4 text-center">
                            ...
                            <p class="text-[10px] text-slate-400 font-bold mt-1">PNG, JPG atau WEBP (Maks. 2MB)</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="group">
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-1 group-focus-within:text-indigo-600 transition-colors">Kata Sandi (Password)</label>
                                <div class="relative">
                        ...
                        <div class="space-y-8">
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-5 ml-1">Pilih Peran (Roles)</label>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 p-6 bg-slate-50 rounded-3xl border border-slate-100 shadow-inner max-h-[280px] overflow-y-auto custom-scrollbar">
                                    <?php foreach($roles as $role): ?>
                                        <label class="relative flex items-center p-3 rounded-xl bg-white border border-slate-100 cursor-pointer hover:border-indigo-200 hover:shadow-sm transition-all group/role">
                                            <input type="checkbox" name="roles[]" value="<?= $role['id'] ?>" class="w-5 h-5 text-indigo-600 bg-slate-100 border-none rounded-lg focus:ring-indigo-500/20 transition-all">
                                            <span class="ml-3 text-xs font-black text-slate-700 uppercase tracking-wider group-hover/role:text-indigo-600 transition-colors"><?= $role['name'] ?></span>
                                        </label>
                                    <?php endforeach; ?>
                                </div>
                                <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest mt-3 ml-1">* Pilih minimal satu peran untuk memberikan hak akses.</p>
                            </div>
                            ...
                            <div class="flex items-center gap-4 p-6 bg-slate-50 rounded-3xl border border-slate-100 shadow-inner">
                                ...
                                <div>
                                    ...
                                    <span class="block text-[9px] font-bold text-emerald-600 uppercase tracking-widest mt-0.5">Pengguna dapat masuk ke sistem</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-16 pt-10 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-end gap-4">
                        <button type="reset" class="w-full sm:w-auto px-8 py-4 text-xs font-black text-slate-400 uppercase tracking-[0.2em] hover:text-rose-500 transition-all">Batalkan Pengisian</button>
                        <button type="submit" class="w-full sm:w-auto px-10 py-4 bg-indigo-600 text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] shadow-xl shadow-indigo-100 hover:bg-indigo-700 hover:-translate-y-1 hover:shadow-indigo-200 transition-all">Simpan Pengguna</button>
                    </div>
                </form>
            </div>
        </div>
