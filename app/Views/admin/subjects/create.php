...
<?= $this->section('content') ?>
        ...
        <div>
            <nav class="flex mb-3 lg:mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3 text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em]">
                    ...
                    <li class="flex items-center">
                        <svg class="w-3 h-3 text-slate-300 mx-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="text-indigo-600 uppercase font-black">Tambah Mapel</span>
                    </li>
                </ol>
            </nav>
            <h1 class="text-2xl md:text-3xl lg:text-4xl font-black text-slate-800 tracking-tight leading-tight uppercase"><?= $title ?></h1>
            <p class="text-slate-500 mt-2 font-medium text-xs lg:text-sm">Daftarkan mata pelajaran baru ke dalam kurikulum akademik.</p>
        </div>
        <div>
    ...
    <div class="pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <form action="<?= base_url('subjects/store') ?>" method="post" class="space-y-6 lg:space-y-8">
            <?= csrf_field() ?>
            ...
            <div class="bg-white rounded-2xl lg:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-10">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative group">
                                    ...
                                    <input class="w-full pl-12 pr-6 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all outline-none font-bold text-slate-800 placeholder:text-slate-300 text-sm" 
                                           id="name" name="name" type="text" placeholder="Contoh: Matematika" value="<?= old('name') ?>" required>
                                </div>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative group">
                                    ...
                                    <input class="w-full pl-12 pr-6 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all outline-none font-bold text-slate-800 placeholder:text-slate-300 text-sm" 
                                           id="kkm" name="kkm" type="number" min="0" max="100" placeholder="Contoh: 75" value="<?= old('kkm', 70) ?>" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <label class="relative flex items-center justify-center p-4 rounded-xl lg:rounded-2xl border-2 border-slate-100 cursor-pointer hover:border-indigo-100 transition-all group overflow-hidden">
                                        <input type="radio" name="category" value="Wajib" class="peer absolute opacity-0" <?= old('category', 'Wajib') == 'Wajib' ? 'checked' : '' ?>>
                                    ...
                                    <label class="relative flex items-center justify-center p-4 rounded-xl lg:rounded-2xl border-2 border-slate-100 cursor-pointer hover:border-amber-100 transition-all group overflow-hidden">
                                        <input type="radio" name="category" value="Muatan Lokal" class="peer absolute opacity-0" <?= old('category') == 'Muatan Lokal' ? 'checked' : '' ?>>
                            ...
                            <div class="space-y-3">
                                ...
                                <textarea class="w-full px-5 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all outline-none font-bold text-slate-800 placeholder:text-slate-300 text-sm min-h-[100px] lg:min-h-[110px]" 
                                          id="description" name="description" placeholder="Berikan gambaran umum mengenai mapel ini..."><?= old('description') ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Submit Panel -->
                <div class="p-6 lg:p-8 border-t border-slate-50 bg-slate-50/30 flex items-center justify-end">
