...
<?= $this->section('content') ?>
        ...
        <div>
            <nav class="flex mb-3 lg:mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3 text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em]">
                    ...
                    <li class="flex items-center">
                        <svg class="w-3 h-3 text-slate-300 mx-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="text-indigo-600 uppercase font-black">Tambah Periode</span>
                    </li>
                </ol>
            </nav>
            <h1 class="text-2xl md:text-3xl lg:text-4xl font-black text-slate-800 tracking-tight leading-tight uppercase"><?= $title ?></h1>
            <p class="text-slate-500 mt-2 font-medium text-xs lg:text-sm">Input periode akademik baru ke dalam sistem.</p>
        </div>
        <div>
    ...
    <div class="pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <form action="<?= base_url('academic-years/store') ?>" method="post" class="space-y-6 lg:space-y-8">
            <?= csrf_field() ?>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative group">
                                ...
                                <input class="w-full pl-12 pr-6 py-3 lg:py-4 rounded-xl lg:rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all outline-none font-bold text-slate-800 placeholder:text-slate-300 text-sm" 
                                       id="year" name="year" type="text" placeholder="Contoh: 2025/2026" value="<?= old('year') ?>" required>
                            </div>
                        </div>
                    ...
                    <div class="p-6 lg:p-8 bg-indigo-50/50 rounded-2xl lg:rounded-[2rem] border border-indigo-100 space-y-4">
                        ...
                        <div class="relative group">
                            <select class="w-full px-5 py-3 lg:py-4 rounded-xl border border-indigo-200 focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 transition-all outline-none font-bold text-indigo-900 text-sm bg-white appearance-none" id="status" name="status">
                                ...
                                <option value="Active" <?= old('status') == 'Active' ? 'selected' : '' ?>>AKTIFKAN SEKARANG</option>
                            </select>
                        ...
                        <div class="flex items-start gap-4 p-4 bg-white/50 rounded-xl border border-indigo-100">
                            <div class="mt-0.5 text-indigo-500">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            </div>
                            <p class="text-[10px] font-bold text-indigo-600 leading-relaxed uppercase tracking-tight">
                                <span class="text-rose-500 font-black">PENTING:</span> Mengaktifkan periode ini akan secara otomatis menonaktifkan semua periode akademik lainnya guna menjaga konsistensi data sistem.
                            </p>
                        </div>
                    </div>
                </div>
